...
function makeHelpers(pptx, footerLabel) {
  let pageN = 0;
  ...
  function footer(s, dark) {
    ...
    s.addText(footerLabel, {
      x: 0.6, y: 7.14, w: 9, h: 0.3, fontFace: FONT, fontSize: 9,
  ...
  function header(s, kicker, title, opts = {}) {
    ...
    if (sub) {
      s.addText(sub, {
        x: 0.6, y: 1.55, w: 12.1, h: 0.55, fontFace: FONT, fontSize: 12, color: MUTED,
  ...
  return { newSlide, footer, header, iconCircle, card, pillBadge, bullets, tableSlide, phaseFlow };
}

module.exports = {
  makeDeck, makeHelpers, A,
