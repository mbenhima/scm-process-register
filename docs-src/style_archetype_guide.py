# -*- coding: utf-8 -*-
"""Generic styling script for the archetype-guide series (Cultural, Automation,
BPR, QMS, Operating Model, Compliance, Training & Skills). Usage:

    python3 style_archetype_guide.py <in.docx> <out.docx> "<header text>"

Applies the same title-block/heading/table styling, TOC repositioning, and a
landscape section spanning Part 4 (with portrait resuming at Part 5, if a
Part 5 heading exists -- otherwise landscape runs to the end of the doc).
"""
import copy
import sys
import warnings
...
IN_DOCX, OUT_DOCX, HEADER_TEXT = sys.argv[1], sys.argv[2], sys.argv[3]
...
if sdt is not None:
    body.remove(sdt)
    title_end_p = d.paragraphs[5]._p  # last title-block paragraph
...
LABEL_PREFIXES = ('Detailed description', 'Trigger', 'Timeline impact',
                   'Recovery tasks', 'Outputs', 'RACSI', 'Cohort', 'Why this window')
for p in d.paragraphs:
    text = p.text.strip()
    if not text or not p.runs:
        continue
    if p.runs[0].bold and any(text.startswith(prefix) for prefix in LABEL_PREFIXES):
        for r in p.runs:
...
if p_before_part4 is not None:
    ...
    if p_before_part5 is not None:
        ...
        pgSz.set(qn('w:h'), str(min(w, h)))
        pgSz.set(qn('w:orient'), 'landscape')
        print('WARNING: no Part 5 heading -- landscape runs to end of document')
else:
    print('WARNING: no Part 4 heading -- skipping landscape section')
