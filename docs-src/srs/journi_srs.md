...
Out of scope: pixel-level visual design beyond the screenshots in Section 10.7 and the palette tokens in Section 5.1 — the palette is POWERACT Consulting's own "Brand & Graphical Chart Guide" (v1.0), adopted directly in `journi/tailwind.config.js` and `journi/src/index.css` (Section 5.1) rather than reproduced as a separate artifact; the illustrative seed-data narratives' full prose form (Section 9 and 10.6 give structured/condensed content; full narratives live in the Master User Guide), and any integration with a specific third-party PMO, HRIS, or LMS system (journi exposes CSV export and a documented API surface as its integration points, but ships no pre-built connector).
...
- **Visual identity**: journi's Tailwind color tokens (`journi/tailwind.config.js`) are pinned to the exact palette in POWERACT Consulting's own "Brand & Graphical Chart Guide" (v1.0): Orange Primary `#F8931D` and Orange Deep `#E07B00` (`brand-600`/`brand-700` — buttons, active navigation, links, KPI numbers, table header fills) as the one accent color, used sparingly and purposefully, against a light background (`surface` `#FDFDFC`) with charcoal-grey chrome and text — Grey Dark `#3A3A3C`, Grey Ink `#58595B`, Grey Medium `#808184`, Grey Light `#F2F2F3`, Grey Line `#E3E3E4` — never black. The guide's five-color semantic status scale (Section 6.2 of the brand guide; red `#F4C7C3` through dark green `#B6D7A8`) is reproduced as the light-tint shades of Tailwind's own `red`/`amber`/`yellow`/`emerald` palettes, so the shared `Badge`, `StatCard`, and `ProgressBar` components render on-brand without per-call-site overrides; multi-series charts (the WBS/Gantt's three workstreams, the Journey Map's event types) use brand orange plus the guide's muted blue `#3A6EA5` and muted green `#5AA469` rather than a fourth competing hue. journi's own logo mark (`journi/public/journi-logo.svg` — a circular orange/blue people-and-cycle icon, distinct from POWERACT's own consulting-firm wordmark) appears in the Sidebar header and on the Login screen, always on a white background, consistent with the guide's general logo-placement good practice even though the guide's specific POWERACT wordmark rules do not apply to journi's own product mark. `journi/src/index.css`'s shared component layer (`.card`, `.btn-*`, `.input`, `.badge`, `.label`) is the single place these tokens are consumed; no module page hardcodes a color outside this layer (Section 8.7, NFR-16).
