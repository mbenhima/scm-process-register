...
d = docx.Document('spec-pandoc.docx')

# ---- Title block (paragraphs 0-6) ----
titles = d.paragraphs[0:7]
sizes = [40, 16, 24, 20, 13, 13, 13]
bolds = [True, False, True, True, False, False, False]
italics = [False, True, False, False, False, False, False]
for p, size, bold, italic in zip(titles, sizes, bolds, italics):
    ...
    for r in p.runs:
        ...
        r.bold = bold
        r.italic = italic
        r.font.color.rgb = H1_COLOR if size == 40 else RGBColor(0x33, 0x33, 0x33)
d.paragraphs[0].space_before = Pt(180)
...
hp.text = 'journi — Functional & Module Specification'
...
d.save('spec-styled.docx')
print('wrote spec-styled.docx')
