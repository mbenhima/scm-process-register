...
| Week | Project Manager (PM) Track | Change Manager (CM) Track | journi Entry --- What to Type In | Exception |
|---|---|---|---|---|
| **Week 12** | **[PM]** Stand up the configuration environment; begin configuration sprint 1 against the signed-off design principles (Task 1, Step 1). | **[CM]** Monitor the last FAQ-channel questions from Design; hand off the champion nominee list to Build's briefing plan. | M21 (Field Notes) --- type in: Category: Handoff · Title: "FAQ Channel Closed Out; Champion List Handed to Build" · Logged by: Driss El Amrani · Body: "Last Design-phase FAQ questions answered and the channel closed out. Champion nominee list (Amal Ferhati, Yassine Bouhali, Rania Idrissi) handed off to Build's Week 16 briefing plan." · Related module: M4 (Stakeholder Mapping). PM's own config sprint 1 decisions are tracked separately on M17 (WBS & Gantt). | --- |
