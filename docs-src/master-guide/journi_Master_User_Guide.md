...
Version 1.0 (Draft --- Part 1 of 5) · August 2026 · Confidential
...
- [Part 1 --- Tenant and Admin Setup](#part-1)
  ...
  - [1.2 The Scenario Organization: Veyra Group](#p1-2)
...
- *Part 2 --- Module-by-Module Feature Tour (all 20 modules) --- in progress, next installment*
- *Part 3 --- All 16 End-to-End Process Walkthroughs --- planned*
- *Part 4 --- Change Management Scenario Library --- planned*
- *Part 5 --- Alerts and Analytics Reference --- planned*
...
It is organized in five parts, delivered as staged installments given its size:

- **Part 1 (this installment) --- Tenant and Admin Setup.** How a brand-new journi tenant is actually built, from the License record through the first Change Management Project, using a new scenario organization created for this guide: **Veyra Group**.
- **Part 2 --- Module-by-Module Feature Tour.** All 20 modules, each demonstrated against Veyra Group's real, growing data set, with CRUD and RBAC behavior called out per role.
- **Part 3 --- All 16 End-to-End Process Walkthroughs.** Every process in the End-to-End Process Catalogue --- the 4 core Change Management chains, the 4 cross-cutting loops, and the 8 transformation-type lifecycles --- each walked through step by step against Veyra Group's data.
...
|---|---|
| Group | Veyra Group |
| Organization | Veyra Manufacturing Europe |
...
| Employees | 3,200 |
| Sites | Porto Plant; Valencia Plant; Lisbon HQ |
...
4. There is no inline edit form for the License fields --- the panel is populated either by the platform default or by uploading a `.lic` file (a JSON file carrying `version`, `companyId`, `companyName`, `hardwareId`, `expiryDate`, `maxUsers`, `plan`, `features`, `issueDate`, and `signature`). For Veyra Group, the Super Admin uploads a `.lic` file with:
   - `companyName`: "Veyra Group"
   - `plan`: "professional"
   - `maxUsers`: 60
   - `features`: the feature flags Veyra Group's contract enables (for example `core_cm_modules`, `wbs_gantt`, `ai_use_case_library`, `process_registry_m18`)
...
2. Click **+ Group**, name it "Veyra Group", save.
3. Click **+ Organization**. Fill in:
   - Name: "Veyra Manufacturing Europe"
   - Group: Veyra Group
   - Type (sector): Manufacturing
   - Employees: 3200
   - Sites (comma-separated): "Porto Plant, Valencia Plant, Lisbon HQ"
   - Language(s): "en, fr"
   - Default language: English
...
Open **Module 2 --- Users & Roles**, **Users & Scope** tab, and click **+ Add** for each of the following, seeding Veyra Group's initial team:

| Name | Role | Scope type | Scope | Language |
|---|---|---|---|---|
| Elena Cardoso | Super Admin | Platform | (whole deployment) | English |
| Rui Teixeira | Group Admin | Group | Veyra Group | English |
| Marta Sousa | Org Admin | Organization | Veyra Manufacturing Europe | English |
| Diego Fontes | Change Manager | Project | Veyra ERP Adoption Program *(created in Step 6 below)* | English |
| Inês Cabral | People Manager | Project | Veyra ERP Adoption Program | French |
| CFO, Veyra Group | Sponsor | Project | Veyra ERP Adoption Program | English |
| Board Executive Viewer | Executive | Organization | Veyra Manufacturing Europe | English |
| Bruno Alves | Employee | Project | Veyra ERP Adoption Program | French |
| Sofia Rocha | Practitioner / Contributor | Project | Veyra ERP Adoption Program | English |
...
1. Back in **Module 1**, on the Veyra Manufacturing Europe Organization card, click **+ Main Project**. Fill in:
   ...
   - Duration: 14 months
   - Budget band: "€3.8M"
   - Executive Sponsor: "CFO, Veyra Group"
2. Click **+ CM Project** on the same Organization card. Fill in:
   - Name: "Veyra ERP Adoption Program"
   ...
   - Target population: "All plant and HQ staff (3,200)"
...
With this checklist complete, Veyra Group is a fully operational journi tenant with one live Change Management Project. Part 2 picks up from here and tours all 20 modules against this same, now-real data set.

---

*End of Part 1. Part 2 --- Module-by-Module Feature Tour --- is the next installment of this guide.*
