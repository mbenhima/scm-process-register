      ...
      <table className="w-full text-sm">
        <thead className="bg-brand-50/70 text-brand-800 text-xs uppercase tracking-wide">
          <tr>
            ...
            <th className="text-start px-4 py-2.5">{t('m19_owning_modules')}</th>
          </tr>
        </thead>
        <tbody>
  ...
  return (
    <div className="card p-4 space-y-2">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Badge tone={KIND_TONE[e2e.kind]}>{t(`m19_kind_${e2e.kind}`)}</Badge>
      </div>
      ...
      {e2e.trigger && (
        <p className="text-xs text-ink/60">
          <strong className="text-ink/80">{t('m19_trigger')}:</strong> {e2e.trigger}
        </p>
      )}
      {e2e.terminalState && (
        <p className="text-xs text-ink/60">
          <strong className="text-ink/80">{t('m19_terminal')}:</strong> {e2e.terminalState}
        </p>
      )}
      ...
      {e2e.kind === 'type' && (
        <div className="grid sm:grid-cols-2 gap-2 pt-1 text-xs">
          <p className="text-ink/60">
            <strong className="text-ink/80">{t('m19_sipoc_supplier')}:</strong> {e2e.sipocSupplier.join(', ')}
          </p>
          <p className="text-ink/60">
            <strong className="text-ink/80">{t('m19_sipoc_customer')}:</strong> {e2e.sipocCustomer.join(', ')}
          </p>
          <p className="text-ink/60 sm:col-span-2">
            <strong className="text-ink/80">{t('m19_phase_template')}:</strong> {e2e.phaseTemplateId} — {phaseTemplateById[e2e.phaseTemplateId]?.name}
          </p>
        </div>
      )}
      {e2e.relatedModules && (
...
function E2ETab({ data }) {
  ...
  return (
    <div className="space-y-6">
      {kinds.map((kind) => (
        <div key={kind}>
          <h3 className="text-sm font-semibold text-brand-950 mb-2">{t(`m19_kind_${kind}`)}</h3>
...
function RacsiTab({ data, canEdit }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-ink/50">{t('m19_racsi_legend')}</p>
      {!canEdit && <p className="text-xs text-ink/40 italic">{t('m19_racsi_readonly')}</p>}
      <div className="card overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-brand-50/70 text-brand-800 text-xs uppercase tracking-wide">
            <tr>
