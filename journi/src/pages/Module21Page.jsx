        ...
        <div key={j.id} className="card p-4 space-y-2">
          ...
          {j.predecessorId && (
            <p className="text-xs text-ink/50">
              {t('m21_specializes')}: {j.predecessorId} — {byId[j.predecessorId]?.name}
            </p>
          )}
          <p className="text-sm text-ink/70">{j.description}</p>
          <div className="grid sm:grid-cols-2 gap-x-4 gap-y-1 text-xs text-ink/60">
            <p><strong className="text-ink/80">{t('m21_trigger')}:</strong> {j.trigger}</p>
            <p><strong className="text-ink/80">{t('m21_audience')}:</strong> {j.audience}</p>
            <p><strong className="text-ink/80">{t('m21_duration')}:</strong> {j.duration}</p>
            <p><strong className="text-ink/80">{t('m21_owner')}:</strong> {j.ownerRole}</p>
          </div>
...
function TouchpointsTab({ project, canEdit }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {!project && <p className="text-xs text-ink/40 italic">{t('m21_select_project_log')}</p>}
        ...
        <table className="w-full text-sm">
          <thead className="bg-brand-50/70 text-brand-800 text-xs uppercase tracking-wide">
            <tr>
              ...
              <th className="text-start px-4 py-2.5">{t('m21_touchpoint')}</th>
              <th className="text-start px-4 py-2.5">PDCA</th>
              <th className="text-start px-4 py-2.5">{t('m21_day')}</th>
              <th className="text-start px-4 py-2.5">{t('m20_owner')}</th>
              <th className="text-start px-4 py-2.5">{t('m21_success_criteria')}</th>
...
function DashboardsTab({ project }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-ink/50">{t('m21_dashboard_desc')}</p>
      <div className="grid md:grid-cols-2 gap-3">
        {journeyDashboards.map((d) => (
          <div key={d.id} className="card p-4 space-y-2">
            ...
            {d.live === 'JRN-01' && project && (
              <div className="rounded-lg bg-brand-50/60 p-2.5">
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="text-brand-800 font-medium">{t('m21_live_metric')}: {t('m21_touchpoint_completion')} ({project.name})</span>
            ...
            {d.live === 'charterActions' && project && (
              <div className="rounded-lg bg-brand-50/60 p-2.5">
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="text-brand-800 font-medium">{t('m21_live_metric')}: {t('m21_charter_completion')} ({project.name})</span>
                  <span className="text-brand-800 font-semibold">{charterRate}%</span>
                </div>
                <ProgressBar value={charterRate} />
              </div>
            )}
            {d.live && !project && <p className="text-xs text-ink/40 italic">{t('m21_select_project_log')}</p>}
            <p className="text-sm text-ink/70">{d.description}</p>
            <div className="grid sm:grid-cols-2 gap-x-4 gap-y-1 text-xs text-ink/50">
              <p><strong className="text-ink/70">{t('m21_kpis')}:</strong> {d.journeyKpis}</p>
              <p><strong className="text-ink/70">{t('m21_audience')}:</strong> {d.audience}</p>
              <p><strong className="text-ink/70">{t('m21_visualisation')}:</strong> {d.visualisationTypes}</p>
              <p><strong className="text-ink/70">{t('m21_linked_report')}:</strong> {d.linkedReport}</p>
            </div>
            {!d.live && (
              <p className="text-xs text-ink/40 italic">{t('m21_reference_only')}</p>
            )}
          </div>
        ))}
      </div>
...
function ContextOverlayTab() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-ink/50">{t('m21_overlay_desc')}</p>
      <div className="space-y-3">
