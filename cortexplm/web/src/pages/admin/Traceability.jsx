...
export default function Traceability() {
  ...
  const rows = useMemo(() => (data || []).map((r) => ({ ...r, group: groupOf(r.id), trace: TRACE[groupOf(r.id)] })), [data]);
  if (error) return <div className="page"><ErrorNote error={error} /></div>;
  if (!data) return <div className="page"><Skeleton h={500} /></div>;
  const sections = [...new Set(rows.map((r) => r.section))];
  const traced = rows.filter((r) => r.trace).length;
  ...
  return (
    <div className="page">
      ...
      <div className="grid kpis" style={{ marginBottom: 24 }}>
        <Kpi value={`${traced}/${rows.length}`} label={t('Requirements traced to a screen and a module')} />
      ...
      <Card>
        <DataTable csvName="srs_traceability" pageSize={40} rows={rows.filter((r) => !section || r.section === section)}
          ...
          columns={[
            ...
            { key: 'verify', label: t('How to verify'), csv: (r) => r.trace?.verify, render: (r) => <span className="xs">{t(r.trace?.verify || '')}</span> },
            { key: 'status', label: t('Status'), csv: (r) => (r.trace ? 'Implemented' : 'Open'), render: (r) => <StatusBadge value={r.trace ? 'Done' : 'Open'}>{t(r.trace ? 'Implemented' : 'Open')}</StatusBadge> },
          ]} />
      </Card>
    </div>
  );
